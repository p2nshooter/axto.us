...
/**
 * Visitor-friendly ad slot for axto.us, governed by the SAME central config as
 * the whole network — fetched from api.ulyah.com and edited only in the
 * ulyah.com admin portal. Renders nothing when the site is OFF (the default)
 * or inside the admin, a labelled preview box when ON without an ad-unit id,
 * and the real AdSense unit once an id is set. Never sticky/interstitial.
 */
interface AdView {
  enabled: boolean;
  clientId: string;
...
export function AdSlot({ placement = "footer", className = "" }: { placement?: string; className?: string }) {
  ...
  return (
    <div className={`my-8 flex min-h-[90px] items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 text-xs text-gray-400 dark:border-gray-700 dark:bg-gray-900 ${className}`}>
